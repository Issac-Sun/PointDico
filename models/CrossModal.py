...
class VisionTransformer(nn.Module):
    def __init__(self, config):
        ...
        if self.backbone_name in clip._MODELS.keys():
            ...
            self.clip = False
            image_model = timm.create_model(self.backbone_name, pretrained=False,checkpoint_path='/home/ubuntu/home/ubuntu/deep-learning/ReCon/jx_vit_base_p16_224-80ecf9dd.pth')
...
class TextTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.template = ['',
                         ...
                         'A wireframe model of ', 'A detailed drawing of ', 'A CAD model of ',
                         'A 2D sketch of ', 'A photorealistic rendering of ', 'A silhouette of ',
                         'A holographic projection of ', 'A digital twin of ', 'A voxel representation of'
                         ]
        self.template_middle = [
            'with detailed texture of ',
            'featuring a simplified structure of ',
            'with a realistic lighting of ',
            'showcasing the geometry of ',
            'with highly polished surfaces of ',
            'in a deconstructed state of ',
            'in a wireframe state of ',
            'under soft shadows on ',
            'with reflective materials of ',
            'showing physical wear and tear on ',
            'as a conceptual design of ',
            'with abstract contours of ',
            'illustrating the topology of ',
            'highlighting the curvature of ',
            'in a raw form of ',
            'emphasizing the symmetries of ',
            'with sharp edges of ',
            'as a miniature representation of ',
            'as a rendered prototype of ',
            'with transparent surfaces of '
        ]
        self.template_last = ['.', ' with white background.', ' with black context.'
                              ' in a natural environment.', ' in a studio setting.',
                              ' under artificial lighting.', ' during the daytime.', ' at night.'
                              ]

    def forward(self, text, index=0):
        if isinstance(text, str):
            middle = random.choice(self.template_middle)
            prompt_text = self.template[index % 20] + middle + text + self.template_last[index // 20]
        else:
            prompt_text = [random.choice(self.template) + random.choice(self.template_middle) + t + random.choice(self.template_last) for t in text]
