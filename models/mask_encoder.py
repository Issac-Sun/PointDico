...
'''
Common modules for Point Transformer, and the same setting as PointMAE
'''
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm ,  with_contrast=False, finetune=False):
        super().__init__()
...
class Mask_Encoder(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        # rand
        self.mask_type = config.encoder_config.mask_type
    ...
    def forward(self, neighborhood, center, noaug = False):
        ...
        group_input_tokens[~bool_masked_pos] = x_vis.reshape(-1, self.trans_dim)
        ...
        # Follow RECON (ICML2023) to add the cls token to the input sequence
        # x_vis = self.blocks(x_vis, pos)
        # transformer
        x = self.blocks(x, pos)
        x = self.norm(x)
        return x[:, 0], x[:, 1], x[:, 2], x[:, 3:], bool_masked_pos,encoder_token
        # # x_vis = self.norm(x_vis)
        # group_input_tokens[~bool_masked_pos] = x.reshape(-1, self.trans_dim)
        # encoder_token = group_input_tokens
        # return encoder_token, bool_masked_pos
