...
"""
We follow PointDif (zheng et al.) to build generator.
We suppose H2Net and DIPNet are better than original PointDif.
...
class H2Net(nn.Module): 
    ...
    def forward(self, patch_fea):
        ...
        # Combine with text feature
        # text_fea = text_feature.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, patch_fea.size(2), 1)  # B F 1 1 -> B F G 1
        combined_fea = torch.cat([patch_fea, global_fea], dim=1)  # B (1024+text_dim) G 1
        ...
        condition_fea = soft_pool2d(combined_fea, kernel_size=[combined_fea.size(2), 1]).squeeze(-1).squeeze(-1)
...
class DenoisingNet(nn.Module):
    ...
    def __init__(self, point_dim, cond_dims, residual):
        ...
        self.layers = nn.ModuleList([
            DIPNet(3, 128, cond_dims+3),
            DIPNet(128, 256, cond_dims+3),
            DIPNet(256, 512, cond_dims+3),
            DIPNet(512, 256, cond_dims+3),
            DIPNet(256, 128, cond_dims+3),
            DIPNet(128, 3, cond_dims+3)
        ])
...
class CPDM(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        # self.text_dims = text_dims  # CLIP 提取的文本特征维度
        ...
        # self.text_alignment_weight = self.generator_config.text_alignment_weight  # 语义损失权重



    def get_loss(self, coords, cond, ts=None, clip_model=None):
        """
        Args:
            coords:        Point cloud, (B, N, 3).
            cond:          Condition (B, F), combines point cloud and text features.
            ts:            Time steps (optional). If None, sampled dynamically.
            text_feature:  Text embeddings from TextTransformer, (B, text_dims).
            clip_model:    Pretrained CLIP model to calculate semantic alignment loss.
        """
        ...
        for i in range(self.interval_nums):
            ...
            # # Calculate semantic alignment loss (CLIP-based)  
            # # We do not use CLIP model here, but you can uncomment the following lines if needed.
            # if clip_model is not None and text_feature is not None:
            #     # Extract point cloud embeddings using CLIP image encoder
            #     point_cloud_feature = clip_model.encode_image(noisy_coords.permute(0, 2, 1))  # Convert to (B, 3, N)
            #     point_cloud_feature = point_cloud_feature / point_cloud_feature.norm(dim=-1, keepdim=True)  # Normalize
            #
            #     # Normalize text embeddings
            #     text_feature = text_feature / text_feature.norm(dim=-1, keepdim=True)
            #
            #     # Cosine similarity loss (maximize similarity between text and point cloud features)
            #     alignment_loss = -torch.mean(torch.sum(point_cloud_feature * text_feature, dim=-1))
            #     semantic_alignment_loss += alignment_loss * (1.0 / self.interval_nums)
